...
def compare(product_id: int, db: Session = Depends(get_db)):
    ...
    gem = _gem_listing([l for l in product.listings])
    gem_price = float(gem.price) if gem and gem.price else None
    ...
    # Calculate overall best price across ALL listings (including GeM)
    available_listings = [l for l in product.listings if l.price is not None and l.price > 0]
    if available_listings:
        best_overall_listing = min(available_listings, key=lambda l: l.price)
        best_overall_price = float(best_overall_listing.price)
        best_overall_marketplace = best_overall_listing.marketplace.name if best_overall_listing.marketplace else None
    else:
        best_overall_price = None
        best_overall_marketplace = None
    ...
    return ProductCompareOut(
        ...
        best_marketplace=best_overall_marketplace,
        matches=matches,
        gem_price=gem_price,
